...
export async function onRequestGet({ request, env, params }) {
  ...
  const id = params.id;
  // Files come from the lesson library, the website inbox, or an idea-board post.
  const meta = await env.DB.prepare(
    `SELECT filename, type FROM lesson_files WHERE id = ?
     UNION ALL SELECT filename, type FROM submission_files WHERE id = ?
     UNION ALL SELECT filename, type FROM post_files WHERE id = ? LIMIT 1`
  ).bind(id, id, id).first();
